...
export default function PredictInput(props: PredictProps){
    ...
    let dropdown = (<div className="flex flex-shrink m-1 rounded-full hover:bg-gray-300 bg-opacity-50" onClick={() => isVisible(!visible)}><Image className={`flex max-w-[1.5rem] transform transition-all duration-500 ${visible && results.length > 0 ? "-" : ""}rotate-180`} pictures={["/api/icons/arrow.svg"]}/> </div>)
    return(
        <div>
            <div className={`relative flex ${props.className}`}>
                <input type="text" className="px-2 flex flex-grow text-sm w-full" 
                    onKeyDown={(e) => {
                        if(e.key === "Enter"){
                            e.preventDefault();
                            isVisible(false)
            ...
            <div className="relative z-50">
                <ul className="absolute w-full max-h-60 overflow-y-scroll overflow-x-hidden">
